...
TARGET_DB = acct.UAT_VA_CAMPING_MART
table_list = []
...
if __name__ == '__main__':
        ...
        tables_result = search_empty_tables(table_list,not_validate_list)
        ...
        if len(tables_result[0]) > 0:
            ...
            if len(messager['table_nm'].unique()):
                ...
                if not messager[messager['msg_type'].isin(['1 empty_table'])].empty:
                    print("\nTarget DB has following empty table(s):\n")
                ...
                if not messager[messager['msg_type'].isin(['4 duplicates'])].empty:
                    print("\nFollowing column(s) have duplicates rows:\n")
                    for index, row in messager[messager['msg_type'].isin(['4 duplicates'])].iterrows():
                        print ("    "+row["messager"])
                if not messager[messager['msg_type'].isin(['5 column_check'])].empty:
                    print("\nColumn(s) validate result:\n")
                    for index, row in messager[messager['msg_type'].isin(['5 column_check'])].iterrows():
                        print ("    "+row["messager"])
