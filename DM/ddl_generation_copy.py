...
def beauty_empty(statement, column_nm, column_type):
    for _ in 60 - len(column_nm):
        column_nm += ' '
    for _ in 25 - len(column_type):
        column_type += ' '
    return statement.replace('[R_COLUMN_NM]',column_nm).replace('[R_COLUMN_TYPE]',column_type)
        ...
        if sheetname != 'ChangeLog':
            ...
            for row in range(1,sheet.max_row+1):
                if sheet.cell(row=row,column=3).value == table_nm:
                    ...
                    row_count +=1
                    if row_count == 2:
                        ddl += beauty_empty(PK_COLUMN,column_nm,column_type)
