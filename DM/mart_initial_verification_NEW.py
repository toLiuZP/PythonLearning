...
def search_empty_tables(cursor, table_list) -> list:
    ...
    for item in rs_table_list:
        ...
        tb_nm = table_name.lower()

        if re.search(r'_tmp',tb_nm) or re.search(r'_temp',tb_nm) or re.search(r'_bk',tb_nm) or re.search(r'_bk\d{4}',tb_nm) or re.search(r'_bk\d{6}',tb_nm) or re.search(r'_legacy',tb_nm):
