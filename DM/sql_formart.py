...
drop_sql_txt = '''
IF NOT EXISTS(SELECT * from dbo.syscolumns WHERE id=object_id('DBO.F_ORDER_ITEM_TRANSACTION') AND name='ORDER_CREATION_DTM')
BEGIN
	ALTER TABLE F_ORDER_ITEM_TRANSACTION ADD ORDER_CREATION_DTM datetime NULL
	exec sys.sp_addextendedproperty 'MS_Description', 'Order creation date time.', 'schema', 'dbo', 'table', 'F_ORDER_ITEM_TRANSACTION', 'column', 'ORDER_CREATION_DTM'
	PRINT '[INFO] ADD COLUMN [DBO].[F_ORDER_ITEM_TRANSACTION].[ORDER_CREATION_DTM]'
END
'''


res = sqlparse.parse(drop_sql_txt)
stmt = res[0]
stmt.tokens
